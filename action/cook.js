...
module.exports = async (message, args) => {
  ...
  if (message.mentions.users.first()) {
    ...
    const Embed1 = new MessageEmbed()
	  .setColor('RANDOM')
	  .setTitle(message.author.username + " esta cocinando a " + message.mentions.users.first().username)
    ...
    .setColor('RANDOM')
	  .setTitle(message.author.username + " esta cocinando.")
    .setFooter(`Pedido por ${message.author.username}`)
        .setImage(data.url)
