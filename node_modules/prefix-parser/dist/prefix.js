...
exports.DiscordPrefixParser = void 0;
...
class DiscordPrefixParser {
    ...
    parse(text) {
        if (!text.startsWith(this.prefix))
            return null;
        // Handle -h/--help
        const help = new flag_1.HelpFlagArg().parse(text)[0];
        if (help)
            throw Error(this.help());
        const keyedArgs = this.args.map((arg, i) => {
            const _arg = arg;
        ...
        for (let i = 0; i < args.length; i++) {
            ...
            try {
                ...
                const helpArg = utils_1.wrap(arg.help(), '`');
        ...
        return keyedArgs.map(arg => valueIndex[arg.__key__]);
    }
    help() {
        return utils_1.formatDiscordHelp(this.title(), this.usage(), this.example());
    }
    /**
     ...
     */
    title() {
        const command = utils_1.wrap(this.prefix, '`');
        const name = this.name ? utils_1.wrap(this.name, '**') : '';
     ...
     */
    usage() {
        const argsHelp = this.args.map(a => a.help()).map(h => utils_1.wrap(h, '`')).join('  ');
        return [utils_1.wrap(this.prefix, '`'), argsHelp].join('  ');
    }
    /**
...
exports.DiscordPrefixParser = DiscordPrefixParser;
