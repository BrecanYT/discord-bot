...
module.exports = async (message, args) => {
  ...
  if (message.mentions.users.first()) {
    const Embed1 = new MessageEmbed()
	  .setColor('RANDOM')
	  .setTitle(message.author.username + " se enamoro de " + message.mentions.users.first().username)
    ...
    .setColor('RANDOM')
	  .setTitle(message.author.username + " esta enamorado 💕")
    .setFooter(`Pedido por ${message.author.username}`)
        .setImage(data.url)
